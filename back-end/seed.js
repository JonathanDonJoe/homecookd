...
INSERT INTO events 
    (time, address, lat, lng, title, description, host_id, portions, price, tags, dine_in, pick_up)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
`
let date = new Date();
date.setHours(date.getHours() + 1000)
seedData.forEach((arr,i)=>{
    let dbValues = [date, arr.address, arr.lat, arr.lng, arr.title, arr.description, arr.host_id, arr.portions,
